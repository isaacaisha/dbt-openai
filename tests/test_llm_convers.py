...
def test_get_all_conversations_with_data(client, user1, redis_client):
...
def test_convers_head_tail(client, redis_client):
    with client.application.test_request_context():
...
def test_show_story(client, redis_client):
    with client.application.test_request_context():
...
def test_update_like(client, user1, conversation_user1, redis_client):
    with client.application.test_request_context():
...
def test_get_conversations_jsonify(client, redis_client):
    with client.application.test_request_context():
